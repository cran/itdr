...
{
  const char uplo = 'L';
  const double sc1 = 0.5 / (*h), sc2 = -0.25 / (*h);
             ...
             const int* n, const int* p, const int* out, double* Mmat)
{
  ...
  const double sc1x = 0.5 / (*hx), sc2x = -0.25 / (*hx);
  const double sc2y = -0.25 / (*hy);
  const double a = 1.0, b = -1.0;
  const int pp = (*p) * (*p), one = 1;
  double alpha, wij, wsum; 
  double xij[*p], gkxij[*p];
...
{
  const char uplo = 'L';
  const double sc1 = -1.0 / (4.0 * (*h));
  const double b = (1.0 + 0.5 / (*h)) / (*h);
...
{
  const char uplo = 'L';
  const double sc1 = -1.0 / (4.0 * (*hx));
  const double scy = -1.0 / (4.0 * (*hy));
  const double b = (1.0 + 0.5 / (*hx)) / (*hx);
  const double a = b + 1.0;
  const double oned = -1.0;
  const int pp = (*p) * (*p), one = 1;
  double aij, alpha, Asum;
